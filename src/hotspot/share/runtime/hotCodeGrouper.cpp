...
void HotCodeGrouper::initialize() {
  ...
  log_info(hotcodegrouper)("HotCodeGrouper::initialize()");

  if (!HotCodeHeap) {
    log_info(hotcodegrouper)("HotCodeGrouper::initialize() called without HotCodeHeap enabled");
  ...
  if (hot_code_heap == nullptr) {
    log_info(hotcodegrouper)("Hot Code Heap not found in code cache");
    return;
  }
...
void HotCodeGrouper::do_grouping(ThreadSampler& sampler) {
  while (sampler.has_candidates()) {
    ...
    MutexLocker ml_InlineCacheBuffer_lock(InlineCacheBuffer_lock, Mutex::_no_safepoint_check_flag);
    MutexLocker ml_CodeCache_lock(CodeCache_lock, Mutex::_no_safepoint_check_flag);
    ...
    while (relocIter.next()) {
      ...
      nmethod* actual_dest_nm = (nmethod*) dest_nm->method()->code();
